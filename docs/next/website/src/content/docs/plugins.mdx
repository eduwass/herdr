...
max_cols = 100       # applies when the host tab area is <= 100 columns
max_rows = 40        # optional; applies when the host tab area is <= 40 rows
width = "100%"
height = "90%"
border = false
border_style = "single"
...
current terminal size. Percent dimensions and breakpoints are evaluated against
the full tab terminal area, not the tiled pane behind the popup.

Breakpoints are optional `[[panes.popup.breakpoints]]` tables. `max_cols` and
`max_rows` are terminal-cell limits for the host tab area; a breakpoint matches
when every provided limit matches. Matching breakpoints override popup fields
...
A breakpoint can set `width`, `height`, `border`, `border_style`,
