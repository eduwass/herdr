...
`overlay`, `split`, `tab`, `zoomed`, or `popup`.
...
`placement = "popup"` opens an ephemeral floating terminal centered on top of
the tiled layout, similar to tmux `display-popup`. A popup lives **outside** the
tiling tree: it is excluded from split, resize, the zoom toggle, and tab/pane
navigation, and it is **dropped on session restore and live-handoff** (popups
are intentionally not persisted). Closing a popup — with `Esc`, the
`plugin.pane.close` API, or when the inner process exits — returns focus to the
previously focused tiled pane. A popup pane is addressed by its raw pane id
(`p_<workspace>_<id>`, returned in the open response); it has no public pane
number.

Style and size a popup with an optional `[panes.popup]` table in the manifest,
and override any field per call with `plugin.pane.open`'s `popup` object (per-call
fields win field-by-field over the manifest defaults):
...
[panes.popup]
width = "80%"      # percent of the terminal width, or an absolute cell count (e.g. 60)
height = 24         # string "50%" or integer cell count
position = "total_center"  # total_center | relative_center (default: total_center)
border = true       # draw a border (default: true)
border_style = "rounded"  # single | double | rounded | thick (default: single)
border_color = "#89b4fa"  # hex, named, or rgb(r,g,b); defaults to the theme accent
padding = 1         # cells of inner margin between border and content (default: 0)
bg = "#1e1e2e"     # popup background color; defaults to the theme panel background
title = "Board"     # optional border title

[[panes.popup.breakpoints]]
max_cols = 100       # applies when the selected positioning area is <= 100 columns
max_rows = 40        # optional; selected positioning area <= 40 rows
width = "100%"
height = "90%"
position = "relative_center"
border = false
border_style = "single"
border_color = "#cba6f7"
padding = 0
...
All `[panes.popup]` fields are optional. Dimensions accept either a percentage
string (`"80%"`, clamped to the terminal axis) or an absolute cell count
(`60`); they are clamped to a minimum content size and re-centered for the
current terminal size. By default, popup positioning is `total_center`: the
popup is centered against the whole terminal frame, ignoring the sidebar, tab
bar, and any tiled pane geometry. Set `position = "relative_center"` to preserve
the old content-area behavior, where the popup is centered against the pane area
behind it. Percent dimensions and breakpoints are evaluated against the selected
positioning area.

Breakpoints are optional `[[panes.popup.breakpoints]]` tables. `max_cols` and
`max_rows` are terminal-cell limits for the selected positioning area; a
breakpoint matches when every provided limit matches. Matching breakpoints override popup fields
field-by-field in manifest order, so later matching rules win. This gives plugin
authors a simple media-query-style escape hatch for narrow windows and mobile
clients.

A breakpoint can set `width`, `height`, `position`, `border`, `border_style`,
`border_color`, `padding`, `bg`, and `title`; omitted fields keep the current
effective value. Breakpoint `width` and `height` accept the same percentage or
absolute-cell forms as `[panes.popup]`.

Popup commands run inside a normal PTY. Herdr resizes that PTY to the popup's
current inner content rect whenever the host terminal size changes, so full-screen
TUIs and shell scripts can react through normal terminal resize mechanisms such
as `SIGWINCH`, `stty size`, or their TUI framework's resize event. On launch,
Herdr also provides `HERDR_POPUP=1`, `HERDR_POPUP_INITIAL_COLS`,
`HERDR_POPUP_INITIAL_ROWS`, `HERDR_TERMINAL_INITIAL_COLS`, and
`HERDR_TERMINAL_INITIAL_ROWS` for simple scripts that want initial dimensions
without querying the TTY first.
