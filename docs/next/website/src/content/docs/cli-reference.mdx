...
```bash
herdr plugin pane open --plugin ID --entrypoint ID [--placement overlay|split|tab|zoomed|popup] [--workspace ID] [--target-pane PANE] [--direction right|down] [--cwd PATH] [--env KEY=VALUE] [--focus|--no-focus]
  # popup styling (placement = popup):
  [--width NN|NN%] [--height NN|NN%] [--position total_center|relative_center] [--border|--no-border] [--border-style single|double|rounded|thick] [--border-color COLOR] [--padding N] [--bg COLOR] [--title TEXT]
...
new tab, a zoomed pane, or a `popup` — an ephemeral floating centered terminal
on top of the tiled layout (dropped on restore and live-handoff). The popup
styling flags override the manifest `[panes.popup]` defaults for a single call.
Popup `--position total_center` is the default and centers against the whole
terminal frame; `--position relative_center` centers against the pane/content
area behind the popup.
Popup pane PTYs are resized to the popup's current inner content rect as the host
terminal changes size; commands can observe that through normal terminal resize
signals/events. Popup launches also receive `HERDR_POPUP=1`,
`HERDR_POPUP_INITIAL_COLS`, `HERDR_POPUP_INITIAL_ROWS`,
`HERDR_TERMINAL_INITIAL_COLS`, and `HERDR_TERMINAL_INITIAL_ROWS`.
Native non-terminal plugin panes are a later surface.

`--env KEY=VALUE` can be repeated on process-launching commands. It applies to the newly launched process only. Herdr-managed variables such as `HERDR_SOCKET_PATH`, `HERDR_BIN_PATH`, `HERDR_ENV`, `HERDR_WORKSPACE_ID`, `HERDR_TAB_ID`, `HERDR_PANE_ID`, `HERDR_PLUGIN_ID`, `HERDR_PLUGIN_ROOT`, `HERDR_PLUGIN_CONFIG_DIR`, `HERDR_PLUGIN_STATE_DIR`, `HERDR_PLUGIN_ENTRYPOINT_ID`, `HERDR_PLUGIN_CONTEXT_JSON`, `HERDR_POPUP`, `HERDR_POPUP_INITIAL_ROWS`, `HERDR_POPUP_INITIAL_COLS`, `HERDR_TERMINAL_INITIAL_ROWS`, and `HERDR_TERMINAL_INITIAL_COLS` stay authoritative when they conflict with caller-provided env.
...
| `HERDR_POPUP` | Set to `1` inside plugin popup pane processes. |
| `HERDR_POPUP_INITIAL_COLS` | Popup content width in cells at launch. |
| `HERDR_POPUP_INITIAL_ROWS` | Popup content height in cells at launch. |
| `HERDR_TERMINAL_INITIAL_COLS` | Host tab terminal width in cells at popup launch. |
| `HERDR_TERMINAL_INITIAL_ROWS` | Host tab terminal height in cells at popup launch. |
