...
Tokens are `{hostname}`, `{session}` (the session name, `default` when unnamed), `{workspace}`, `{tab}`, `{pane}` (the focused pane's manual name), and `{terminal_title}` (the focused pane's own terminal title with spinner frames stripped). Write `{{` and `}}` for literal braces. A token with no value renders empty.
...
||||||| parent of 3ac32f7 (feat: add opt-in confirmation when closing a pane with a running process)
[ui]
sidebar_width = 32
...
`confirm_close_running` adds a tmux-style confirmation when you close a pane that has a non-shell process running in it, such as a long-running script or a coding agent like Claude. When enabled and the pane's foreground process is something other than its own shell, Herdr prompts `kill pane running <cmd>?` before closing. It is off by default; panes sitting at the shell prompt always close immediately. The prompt only applies to interactive closes (keybinding, context menu): `herdr pane close`, `herdr tab close`, and other API clients close immediately so scripts and agents are never blocked on a modal.
...
Set `pane_border_shows_osc_title = true` to show the agent's live OSC/session title (the task line an agent emits, such as "Friendly greeting") in the pane border. The OSC title takes precedence over the detected agent name, but a manual pane rename always wins over it.
