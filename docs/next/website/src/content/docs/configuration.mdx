...
Set `pane_double_right_click_zoom = true` to toggle zoom when you rapidly right-click twice inside the same pane. The first right-click keeps its normal behavior, such as opening Herdr's pane menu; the second matching right-click closes that menu and toggles zoom.
