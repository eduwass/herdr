...
{"id":"req_plugin_popup","method":"plugin.pane.open","params":{"plugin_id":"example.board","entrypoint":"board","placement":"popup","popup":{"width":"80%","height":"60%","border":true,"border_style":"rounded","border_color":"#89b4fa","padding":1,"bg":"#1e1e2e","title":"Board","breakpoints":[{"max_cols":100,"width":"100%","height":"90%","border":false,"padding":0}]}}}
```
...
`border`, `border_style`, `border_color`, `padding`, `bg`, `title`, and
`breakpoints`. `width` and `height` accept either absolute cell counts or percent
strings such as `"80%"`. Each breakpoint can set `max_cols`, `max_rows`, and the
...
against the full tab terminal area, and Herdr resizes the popup PTY to the
