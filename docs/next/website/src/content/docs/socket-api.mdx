...
`HERDR_PLUGIN_ENTRYPOINT_ID`. Popup pane commands also receive
`HERDR_POPUP=1`, `HERDR_POPUP_INITIAL_COLS`, `HERDR_POPUP_INITIAL_ROWS`,
`HERDR_TERMINAL_INITIAL_COLS`, and `HERDR_TERMINAL_INITIAL_ROWS`.
...
{"id":"req_plugin_popup","method":"plugin.pane.open","params":{"plugin_id":"example.board","entrypoint":"board","placement":"popup","popup":{"width":"80%","height":"60%","position":"total_center","border":true,"border_style":"rounded","border_color":"#89b4fa","padding":1,"bg":"#1e1e2e","title":"Board","breakpoints":[{"max_cols":100,"width":"100%","height":"90%","position":"relative_center","border":false,"padding":0}]}}}
```
...
`placement` overrides the manifest with `overlay`, `split`, `tab`, `zoomed`, or
`popup`. Overlay panes target the active pane. Split and zoomed panes target an
existing pane; tab panes can target a workspace. Popup panes are centered over
the active tab, live outside the tiled layout tree, and are dropped on restore
and live-handoff.

For popup placement, the optional `popup` object accepts `width`, `height`,
`position`, `border`, `border_style`, `border_color`, `padding`, `bg`, `title`, and
`breakpoints`. `width` and `height` accept either absolute cell counts or percent
strings such as `"80%"`. `position` defaults to `total_center`, which centers
against the whole terminal frame regardless of sidebar or tab layout; use
`relative_center` to center against the pane/content area behind the popup. Each
breakpoint can set `max_cols`, `max_rows`, and the
same popup override fields; matching breakpoints are applied in order and later
matches win field-by-field. Popup dimensions and breakpoints are evaluated
against the selected positioning area, and Herdr resizes the popup PTY to the
current inner content rect when the host terminal size changes.

The pane behaves like a normal Herdr pane, but `plugin.pane.focus` and
`plugin.pane.close` only operate on panes opened through the plugin API. Focus
returns `plugin_pane_focused`; close returns `plugin_pane_closed`. Focused popup
panes capture keyboard input until plain `Esc` closes the popup.
