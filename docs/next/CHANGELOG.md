...
- Added a pane context menu action to move a pane into a new tab.
...
- Plugin popup pane sizing now uses the full terminal frame by default instead of the focused tiled pane behind it, so centered popups stay centered even when the sidebar is visible. Plugins can opt into the old content-area behavior with `position = "relative_center"`.
